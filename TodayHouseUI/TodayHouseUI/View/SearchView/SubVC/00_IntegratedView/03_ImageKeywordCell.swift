...
class ImageKeywordCell: UICollectionViewCell, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    
    var imageKeywordData: [IntegratedVcData] = []
...
//        layout.estimatedItemSize = CustomFlowLayout.automaticSize
        ...
        collectionView?.register(ImageKeywordDetailCell.self, forCellWithReuseIdentifier: "ImageKeywordDetailCell")
    ...
    func setupConstraints() {
        let height = (dummy.size(withAttributes: [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 14)]).height + 10) * 2 + 30
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return imageKeywordData.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "ImageKeywordDetailCell", for: indexPath) as? ImageKeywordDetailCell else { return UICollectionViewCell() }
        let data = imageKeywordData[indexPath.row]
...
//        flow.estimatedItemSize = UICollectionViewFlowLayout.automaticSize
        let width = imageKeywordData[indexPath.item].title.size(withAttributes: [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 14)])
...
class ImageKeywordDetailCell: UICollectionViewCell {
